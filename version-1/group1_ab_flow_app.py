...
import pandas as pdimport requestsimport streamlit as stimport streamlit.components.v1 as components

APP_PAGE_TITLE = "🐱 家貓情緒標註系統｜第 1 組"OUTPUT_CSV = Path("hci_cat_annotation_group1.csv")GROUP_ID = "1"BASE_DIR = Path(file).resolve().parentIMAGE_DIR = BASE_DIR / "images"if not IMAGE_DIR.exists():IMAGE_DIR = BASE_DIR.parent / "images"ANNOTATION_DIR = BASE_DIR / "annotations"if not ANNOTATION_DIR.exists():ANNOTATION_DIR = BASE_DIR.parent / "annotations"

SHEET_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbyMDrGh8WRV-ZyuEFY8uzmVASLSm9JEfZC4pqqGg398KFT8uKWBpNXaLO-9NGGqM17vLQ/exec"SHEET_SECRET = "hci_cat_annotation_secret"

IMAGE_SET_1 = [{"image_id": "set1_001", "path": str(ANNOTATION_DIR / "video9_frame_0.jpg")},{"image_id": "set1_002", "path": str(ANNOTATION_DIR / "video61_frame_5.jpg")},{"image_id": "set1_003", "path": str(ANNOTATION_DIR / "video39_frame_14.jpg")},]

IMAGE_SET_2 = [{"image_id": "set2_001", "path": str(ANNOTATION_DIR / "video30_frame_3.jpg")},{"image_id": "set2_002", "path": str(ANNOTATION_DIR / "video10_frame_4.jpg")},{"image_id": "set2_003", "path": str(ANNOTATION_DIR / "00000001_016.jpg")},]

STAGE_PLAN = [{"stage_name": "第一階段","flow_type": "A","flow_name": "特徵導向","photo_set_name": "照片組 1","images": IMAGE_SET_1,"description": "先標註眼睛、耳朵、尾巴、身體姿勢，再選擇最終情緒。",},{"stage_name": "第二階段","flow_type": "B","flow_name": "情緒導向","photo_set_name": "照片組 2","images": IMAGE_SET_2,"description": "先選擇初步情緒，再檢查部位特徵，最後確認或修改情緒。",},]

── 情緒定義（含圖片路徑）──

EMOTION_SCHEMA = {"害怕": {"icon": "😿","definition": "由立即感知到的危險或危險的威脅引起的，表現為警惕和試圖撤退或逃跑。","image": IMAGE_DIR / "fear.png",},"憤怒": {"icon": "😾","definition": "由執行行動/實現目標的願望受挫或資源競爭引起，表現為攻擊性或攻擊威脅。","image": IMAGE_DIR / "anger.png",},"歡樂/玩耍": {"icon": "😺","definition": "表現為非功能性行為，包括運動遊戲、社交遊戲或物件遊戲。","image": IMAGE_DIR / "joy.png",},"滿意": {"icon": "😽","definition": "由需求和願望得到滿足而產生的正向情緒狀態，表現為休息、平靜和親和。","image": IMAGE_DIR / "contentment.png",},"好奇": {"icon": "🐾","definition": "由新奇或顯著刺激引起，表現為注意、定向或探索行為。","image": IMAGE_DIR / "interest.png",},"中性": {"icon": "➖","definition": "不明顯屬於特定情緒，偏中性、休息或日常活動的狀態。","image": None,},}

對應到 EMOTION_OPTIONS 的情緒定義查詢（做彈性映射）

EMOTION_OPTIONS = ["害怕","憤怒","歡樂/玩耍","滿意","好奇","中性","其他／無法判斷",]

EMOTION_ICONS = {"害怕": "😿","憤怒": "😾","滿意": "😽","好奇": "🐾","中性": "➖","其他／無法判斷": "❓","歡樂/玩耍": "😺",}

FEATURE_OPTIONS = {"眼睛": ["睜大","半睜／放鬆","緊閉","瞳孔放大","直視","避免眼神接觸","無法辨識","其他",],"耳朵": ["直立","朝向刺激物","側向","壓平","無法辨識","其他",],"尾巴": ["豎起","水平","放鬆","夾起","壓低僵硬","快速甩動","無法辨識","其他",],"身體／姿勢": ["放鬆","緊繃","壓低","前傾","拱背","炸毛","發抖／僵硬","姿勢變化頻繁","無法辨識","其他",],}

UNCERTAIN_REASONS = ["影像品質不足","線索不足","多種情緒並存","超出現有分類","其他",]

st.set_page_config(page_title=APP_PAGE_TITLE, layout="wide")

st.markdown("""@import url('https://fonts.googleapis.com/css2?family=Noto+Serif+TC:wght@400;600;700&family=Noto+Sans+TC:wght@300;400;500;700&display=swap');
...
@st.dialog("情緒定義")def show_emotion_dialog(emotion_name: str):item = EMOTION_SCHEMA.get(emotion_name)if not item:st.warning("找不到此情緒的定義。")returnicon = item.get("icon", "")st.markdown(f''f'{icon} {emotion_name}',unsafe_allow_html=True,)img_path = item.get("image")if img_path and Path(img_path).exists():st.image(str(img_path), width=260)st.markdown(f"定義： {item['definition']}")

def get_stage_plan():return STAGE_PLAN

def init_state():defaults = {"participant_id": "","page": "intro","stage_index": 0,"image_index": 0,"task_start_time": None,"pending_stage_records": [],"pending_cloud_rows": [],"cloud_sync_attempted": False,"last_save_message": "",}for key, value in defaults.items():if key not in st.session_state:st.session_state[key] = value

def current_stage():return get_stage_plan()[st.session_state.stage_index]

def current_image():stage = current_stage()images = stage["images"]if st.session_state.image_index >= len(images):return Nonereturn images[st.session_state.image_index]

def reset_task_timer():st.session_state.task_start_time = time.time()

def request_scroll_to_top():st.session_state["scroll_to_top"] = True

def do_scroll_to_top_if_needed():if st.session_state.pop("scroll_to_top", False):components.html("""function forceScrollToTop() {try {const doc = window.parent.document;
...
def clear_temp_csv_and_session_records():try:if OUTPUT_CSV.exists():OUTPUT_CSV.unlink()except Exception as e:st.warning(f"暫存 CSV 無法刪除：{e}")
...
def reset_all():st.session_state.page = "intro"st.session_state.stage_index = 0st.session_state.image_index = 0st.session_state.task_start_time = Nonest.session_state.pending_stage_records = []st.session_state.pending_cloud_rows = []st.session_state.cloud_sync_attempted = Falsest.session_state.last_save_message = ""

def append_records_to_google_sheet(records):if not SHEET_WEBHOOK_URL.strip():return False, "尚未設定 SHEET_WEBHOOK_URL，因此只儲存本機 CSV。"
...
DATA_COLUMNS = ["group_id","stage_name","photo_set_name","participant_id","flow_type","image_id","initial_emotion","selected_features","final_emotion","uncertain_reason","uncertain_other_text","confidence","annotation_time","emotion_changed","workload_q1","workload_q2","workload_q3","confidence_q1","confidence_q2","confidence_q3","clarity_score","usefulness_score","intention_score","open_feedback",]

def save_records(records):rows = [{col: record.get(col, "") for col in DATA_COLUMNS} for record in records]df_new = pd.DataFrame(rows, columns=DATA_COLUMNS)
...
def sync_to_cloud():rows = st.session_state.get("pending_cloud_rows", [])if not rows:return True, "沒有需要同步的資料。"
...
def go_previous_page():page = st.session_state.get("page", "intro")
...
def render_back_button():if st.session_state.get("page") != "intro":if st.button("← 上一頁"):go_previous_page()

def render_placeholder(image_id):st.markdown(f'圖片預覽區目前尚未放入實際照片image_id：{image_id}',unsafe_allow_html=True,)

def render_sidebar_image():with st.sidebar:stage = current_stage()image = current_image()st.markdown("### 家貓照片")st.caption(f"{stage['stage_name']}｜版本 {stage['flow_type']}｜{stage['photo_set_name']}")if image is None:st.info("此階段已完成。")returnst.caption(f"image_id：{image['image_id']}")path = image.get("path", "")if path and Path(path).exists():st.image(path, use_container_width=True)else:render_placeholder(image["image_id"])

def render_sidebar_emotion_quickview():"""Sidebar 底部：情緒定義快速查看（只顯示定義與圖片）"""with st.sidebar:st.markdown("---")st.markdown('''📖 情緒定義快速查看',unsafe_allow_html=True,)
...
def build_selected_features(feature_values, feature_other_text):result = {}for group_name, choice in feature_values.items():if choice == "其他":text = feature_other_text.get(group_name, "").strip()result[group_name] = f"其他：{text}" if text else "其他"else:result[group_name] = choice or ""return json.dumps(result, ensure_ascii=False)

def render_feature_selector(prefix):feature_values = {}feature_other_text = {}for group_name, options in FEATURE_OPTIONS.items():st.markdown(f"#### {group_name}")choice = st.radio(f"請選擇{group_name}最符合的觀察結果",options,index=None,key=f"{prefix}feature{group_name}",)feature_values[group_name] = choice or ""if choice == "其他":feature_other_text[group_name] = st.text_input(f"請補充{group_name}其他特徵",key=f"{prefix}feature_other{group_name}",)else:feature_other_text[group_name] = ""st.divider()return feature_values, feature_other_text

def render_uncertain_reason(prefix, final_emotion):if final_emotion != "其他／無法判斷":return "", ""st.markdown("### 其他／無法判斷原因")reason = st.radio("若選擇其他／無法判斷，請記錄原因",UNCERTAIN_REASONS,index=None,key=f"{prefix}_uncertain_reason",)other_text = ""if reason == "其他":other_text = st.text_input("請補充其他原因", key=f"{prefix}_uncertain_other_text")return reason or "", other_text

def build_base_record(stage, image, initial_emotion, selected_features, final_emotion, uncertain_reason, uncertain_other_text, confidence):annotation_time = round(time.time() - st.session_state.task_start_time, 2)if stage["flow_type"] == "B":emotion_changed = str(initial_emotion != final_emotion)else:emotion_changed = ""
...
def go_next_image_or_questionnaire(record):st.session_state.pending_stage_records.append(record)stage = current_stage()st.session_state.image_index += 1st.session_state.task_start_time = Noneif st.session_state.image_index >= len(stage["images"]):st.session_state.page = "stage_questionnaire"request_scroll_to_top()st.rerun()

def render_flow_a(stage, image, prefix):st.markdown("## Step 1：觀看家貓照片")st.write("請先觀察左側照片，再進行部位特徵標註。")
...
def render_flow_b(stage, image, prefix):st.markdown("## Step 1：觀看家貓照片")st.write("請先觀察左側照片，依照整體感受選擇初步情緒。")
...
def likert(item_no, label, key):clean_label = label.rstrip("。")
...
def avg_or_none(values):if any(v is None for v in values):return Nonereturn round(sum(values) / len(values), 2)

def render_stage_questionnaire():stage = current_stage()si = st.session_state.stage_index
...
def render_intro():st.markdown(f'🐱 {APP_PAGE_TITLE.replace("🐱 ", "")}',unsafe_allow_html=True,)st.markdown('一個網頁完成同一組受試者的兩個階段：A 與 B 都會做，但順序與照片組不同。', unsafe_allow_html=True)
...
def render_task():render_sidebar_image()render_sidebar_emotion_quickview()
...
def render_done():# ── 自動嘗試一次雲端同步 ──if not st.session_state.get("cloud_sync_attempted", False):st.session_state["cloud_sync_attempted"] = Truetry:ok, msg = sync_to_cloud()st.session_state["last_save_message"] = msgexcept Exception as e:st.session_state["last_save_message"] = f"CSV 已儲存，但 Google Sheet 同步失敗：{e}"
...
def main():init_state()st.markdown('', unsafe_allow_html=True)
