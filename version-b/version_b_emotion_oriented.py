...
st.set_page_config(page_title="家貓情緒標註系統 - 版本 B", layout="wide")
...
FLOW_TYPE = "B"
FLOW_NAME = "情緒導向"
OUTPUT_CSV = Path("hci_cat_annotation_b.csv")
...
1. 觀看家貓照片。  
2. 依照整體感受選擇初步情緒。  
3. 標註眼睛、耳朵、尾巴、身體姿勢等部位特徵。  
4. 再次確認是否維持或修改最終情緒。  
5. 若選擇「其他／無法判斷」，需進一步選擇原因。  
6. 填寫標註信心程度。  
""")
...
def render_annotation():
    ...
    st.markdown('<div class="hint-box">流程 B：先依整體感受選擇初步情緒，再回頭檢查支持線索，最後確認是否維持或修改最終情緒。</div>', unsafe_allow_html=True)

    st.markdown("## Step 1：觀看家貓照片")
    st.write("請先觀察左側照片，再依照整體感受選擇初步情緒。")

    st.markdown("## Step 2：依照整體感受選擇初步情緒")
    initial_emotion = st.radio(
        "初步情緒",
        EMOTION_OPTIONS,
        index=None,
        key=f"initial_emotion_{st.session_state.current_index}",
    )

    st.markdown("## Step 3：標註支持判斷的部位特徵")
    selected, other_text = render_feature_selector()

    st.markdown("## Step 4：再次確認是否維持或修改最終情緒")
    final_emotion = st.radio(
        "最終情緒",
        EMOTION_OPTIONS,
        index=EMOTION_OPTIONS.index(initial_emotion) if initial_emotion in EMOTION_OPTIONS else None,
        key=f"final_emotion_{st.session_state.current_index}",
    )
    emotion_changed = "" if not initial_emotion or not final_emotion else str(initial_emotion != final_emotion)
    ...
    required_ok = bool(initial_emotion) and bool(final_emotion) and confidence is not None and questionnaire["is_complete"]
